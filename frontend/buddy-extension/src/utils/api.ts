...
export async function generateQuiz(
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(json?.message || `Quiz generation failed (${res.status})`);
  return json;
}
...
export async function getChatHistoryApi() {
  ...
  const json = await res.json();
  if (!res.ok || !json.success)
    throw new Error(json?.message || `Load history failed (${res.status})`);
  return json as {
    success: true;
...
export async function clearChatApi() {
  ...
  const json = await res.json();
  if (!res.ok || !json.success)
    throw new Error(json?.message || `Clear chat failed (${res.status})`);
  return json as { success: true; message: string };
}

// Code Review API
export async function reviewCodeApi(code: string) {
  const res = await fetch(`${BACKEND_URL}/api/ai/review-code`, {
    method: "POST",
    headers: await authHeaders(),
